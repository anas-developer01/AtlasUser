...
import { getFCMToken, notificationListener, updateDeviceTokenWithBackend } from "../../utils/pushNotification";
...
const Home = (props) => {
    ...
    useEffect(() => {
        getData();
        const runFCMAndSaveToken = async () => {
            await getFCMToken();
            if (user?.token) {
                await updateDeviceTokenWithBackend(user.token);
            }
        };
        runFCMAndSaveToken();
        const unsubscribeNotifications = notificationListener();
        const willFocusSubscription = props?.navigation.addListener('focus', () => {
            getData();
            if (user?.token) updateDeviceTokenWithBackend(user.token);
        });
        return () => {

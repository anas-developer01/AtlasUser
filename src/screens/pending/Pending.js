...
const Pending = (props) => {
    ...
    const getData = async () => {
        setisLoading(true);
        const Res = await tickets(user?.token,props?.route?.params?.screen === 'Pending'?'pending':'in_progress');
        console.log('RES',Res);
        setisLoading(false);
        setPendingData(Res?.data?.records?Res?.data?.records:[]);
    }
    ...
    return(
        <View style={{flex:1}}>
            ...
            <Text style={{color:Black,fontSize:16,width:W(63),fontFamily:'Poppins-Medium',marginLeft:H(2),marginTop:H(1)}}>{props?.route?.params?.screen}</Text>
            </View>
            ...
            {pendingData?.length == 0 ? (
                <Text style={{color:Black,fontSize:16,marginLeft:H(18),marginTop:H(5)}}>No Data Found</Text>
            ):null}
            </View>}
